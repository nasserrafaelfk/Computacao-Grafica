...
void verificaColisaoSemParar()
{
 ...
 /*                                            
    if(tiro_inimigo.pos_y == alt_nav_prin)           // identifica colisão do tiro inimigo com o ash e tira vida
    {
        if(quantidade_vidas > 0)
        {
            dx = tiro_inimigo.pos_x - (nave_principal.pos_x + espaco_x);
            dy = tiro_inimigo.pos_y - (nave_principal.pos_y + espaco_y);

            distancia = sqrt(dx * dx + dy * dy);

            if(distancia < (larg_tiro_inimigo/2 + nave_principal.larg_nav/2))
            {
                quantidade_vidas--;
